...
namespace StarTracker {
	...
	ExperimentalView::ExperimentalView(void* nativeWindowHandle) noexcept : Core::View{ nativeWindowHandle }, invertedTextureCoordinates{ false }, camera{}, shader{}, frameBuffer{}, model{} {
	...
	void ExperimentalView::OnUpdate(float deltaTime) noexcept {
        ...
        if (ImGui::Begin("3D-Model-Viewer")) {
            ...
            ImGui::Image(reinterpret_cast<void*>(static_cast<intptr_t>(frameBuffer->GetNativeTextureHandle())), ImGui::GetContentRegionAvail(), { 0, 1 }, { 1, 0 });
		}
		ImGui::End();
    }
